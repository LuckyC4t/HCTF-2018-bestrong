...
func (alg *EcdhAesKW) WrapNewKey(cekSizeBits int, key interface{}, header map[string]interface{}, pk *ecdsa.PrivateKey) (cek []byte, encryptedCek []byte, err error) {
	var kek []byte

	if kek,_,err=alg.ecdh.WrapNewKey(alg.keySizeBits, key, header, pk);err!=nil {
		return nil,nil,err
	}

	return alg.aesKW.WrapNewKey(cekSizeBits,kek,header, pk)
}
