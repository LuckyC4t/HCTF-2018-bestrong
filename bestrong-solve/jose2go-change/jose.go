...
import (
	...
	"bestrong-solve/jose2go-change/compact"
)
...
type JwaAlgorithm interface {
	WrapNewKey(cekSizeBits int, key interface{}, header map[string]interface{}, pk *ecdsa.PrivateKey) (cek []byte, encryptedCek []byte, err error)
...
func Encrypt(payload string, alg string, enc string, key interface{}, pk *ecdsa.PrivateKey, options ...func(*JoseConfig)) (token string, err error) {
	return EncryptBytes([]byte(payload), alg, enc, key, pk, options...)
}
...
//
// It returns 5 parts encrypted JWT token as string and not nil error if something went wrong.
func EncryptBytes(payload []byte, alg string, enc string, key interface{}, pk *ecdsa.PrivateKey, options ...func(*JoseConfig)) (token string, err error) {
	...
	return encrypt(byteContent, cfg.Headers, key, pk)
}
...
func encrypt(payload []byte, jwtHeader map[string]interface{}, key interface{}, pk *ecdsa.PrivateKey) (token string, err error) {
	var ok bool
	...
	if cek, encryptedCek, err = keyMgmtAlg.WrapNewKey(encAlg.KeySizeBits(), key, jwtHeader, pk); err != nil {
		return "", err
	}
